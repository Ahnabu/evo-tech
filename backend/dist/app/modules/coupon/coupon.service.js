...
const createCoupon = async (couponData) => {
    ...
    const existingCoupon = await coupon_model_1.Coupon.findOne({ code: couponData.code?.toUpperCase() });
    if (existingCoupon) {
...
const validateCoupon = async (code, userId, orderAmount) => {
    ...
    if (coupon.minimumOrderAmount && orderAmount < coupon.minimumOrderAmount) {
        throw new AppError_1.default(http_status_1.default.BAD_REQUEST, `Minimum order amount of ৳${coupon.minimumOrderAmount} required`);
    }
