...
const ourClientsSlice = createSlice({
    ...
    initialState,
    reducers: {
        setOurClientsList: (state, action: PayloadAction<{ data: OurClientsDisplayType[]; fetchedStatus: boolean; }>) => {
            state.allClientItems.data = action.payload.data;
        ...
        addAClientItem: (state, action: PayloadAction<OurClientsDisplayType>) => {
            ...
            const sortedClientItems = [...state.allClientItems.data].sort((a, b) => a.sortorder - b.sortorder);
        ...
        updateAClientItem: (state, action: PayloadAction<OurClientsDisplayType>) => {
            ...
            if (index !== -1) {
                ...
                if (oldSortorder !== newSortorder) {
                    if (newSortorder > oldSortorder) {
                        ...
                        state.allClientItems.data.forEach(clientItem => {
                            if (clientItem.trustedbyid !== updatedClientItem.trustedbyid && 
