...
export function ProductsCount() {
    const { paginationData, isLoading } = useProductsData();
    ...
    const totalCount = paginationData?.total_items || 0;
    ...
    return (
        ...
        >
            <Package className="w-3 h-3 mr-1.5" />
            <span>{totalCount.toLocaleString()} {totalCount === 1 ? 'Product' : 'Products'}</span>
        </Badge>
    );
}
