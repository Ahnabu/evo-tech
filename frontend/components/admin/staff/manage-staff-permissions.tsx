...
export function ManageStaffPermissions({ staffUuid, staffName }: ManageStaffPermissionsProps) {
    ...
    const fetchPermissions = useCallback(async () => {
        ...
        try {
            ...
            const allPermsRes = await axiosInstance.get("/permissions");
            
            // Fetch staff's current permissions
            const staffPermsRes = await axiosInstance.get(`/permissions/staff/${staffUuid}`);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const response = await axiosInstance.put(`/permissions/staff/${staffUuid}`, {
        ...
        <Card>
            ...
            <CardContent className="space-y-6">
                {categories.map(category => {
                    ...
                    const someCategorySelected = categoryPerms.some(p => selectedPermissions.includes(p.code));
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm text-gray-600">
                                        ({categoryPerms.filter(p => selectedPermissions.includes(p.code)).length}/{categoryPerms.length})
                                    </span>
                                </div>
                                <Button
                                    variant="ghost"
