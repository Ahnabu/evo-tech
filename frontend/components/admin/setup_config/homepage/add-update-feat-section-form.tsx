...
import { featuredAddSchema, featuredUpdateSchema, FeaturedAddFormValues, FeaturedUpdateFormValues, FeaturedSectionDisplayType } from '@/schemas/admin/setupconfig/homepage/featuredSections/featuredSchema'
...
interface UpdateFeaturedSectionFormProps extends Omit<FeaturedSectionFormProps, 'mode' | 'sectionData'> {
    sectionData: FeaturedSectionDisplayType;
}

const FeaturedSectionForm = ({ mode = 'create', sectionData, onSuccess }: FeaturedSectionFormProps) => {
    ...
    const onSubmit = async (values: FeaturedAddFormValues | FeaturedUpdateFormValues) => {
        const payload = {
            title: values.title,
            sortorder: parseInt(values.sortorder, 10),
            ...(values.view_more_url && {
                view_more_url: values.view_more_url,
            }),
        }
        ...
        if (response && response.sections_data && response.sections_data.length > 0) {
            toast.success(isUpdate ? `Section updated` : `Section created`);
            // update Redux state with the new section (if created) or edited section (if updated)
            if (isUpdate) {
                // update the entire list since backend reorders all sections
                dispatch(setFeaturedSectionsList({
                    data: response.sections_data,
                    fetchedStatus: true,
                }));
            } else {
                // update the entire list since backend reorders all sections
        ...
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            <FormLabel className="block text-xs font-medium text-left">Section Title*</FormLabel>
                            <FormControl>
                                <Input
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            <FormLabel className="block text-xs font-medium text-left">View More URL (Optional)</FormLabel>
                            <FormControl>
                                <Select
                                    ...
                                    <SelectContent>
                                        {categories.length > 0 ? categories.map((category) => (
                                            <SelectItem key={category.id} value={category.value}>
                                                {/* because the value is the slug of the category, will be used as the URL */}
                                                {category.value}
                                            </SelectItem>
                                        )) :
...
const UpdateFeaturedSectionForm = ({ sectionData }: UpdateFeaturedSectionFormProps) => {
    ...
    return (
        ...
        >
            <FeaturedSectionForm mode="update" sectionData={sectionData} onSuccess={handleSuccess} />
        </EditingDialog>
    );
}
