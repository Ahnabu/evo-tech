...
const ColorSelector = memo(
  ({ colors, selectedColor, onColorSelect }: ColorSelectorProps) => {
        ...
        {parsedColors.map((coloroption, idx) => (
          <Tooltip
            ...
            classNames={{
              ...
              content:
                "min-w-[80px] w-fit max-w-[200px] h-fit py-1 px-3 text-white bg-gradient-to-b from-stone-400 to-stone-800 font-inter text-[11px] sm:text-[12px] leading-5 font-[500]",
            }}
            content={coloroption.name}
            className="w-fit rounded-[10px] bg-transparent capitalize shadow-md"
          >
            <div
              className={`w-6 h-6 sm:w-7 sm:h-7 rounded-full cursor-pointer border-2 border-white ring-2 ${
