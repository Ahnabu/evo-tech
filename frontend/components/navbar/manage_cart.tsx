    ...
    useEffect(() => {
        const fetchCartDB = async () => {
            ...
            if (cartResponse && cartResponse.cartdata) {
                ...
                localStorage.setItem('evoFrontCart', JSON.stringify({ items: cartResponse.cartdata, ctoken: cartResponse.ctoken }));
                // Dispatch a custom event when updating the cart
                const event = new CustomEvent('localStorageChange', {
                    detail: { key: 'evoFrontCart', newValue: { items: cartResponse.cartdata, ctoken: cartResponse.ctoken } }
                });
    ...
    return (
        <>
            <button
                ...
                title={`cart: ${cartData ? (cartData.length > 9 ? `9+` : cartData.length) : '0'} items`}
                ...
                <>
                    <LazyMotion features={LoadMotionFeatures} strict>
                        <AnimatePresence>
                            ...
                            <div className={`z-[350] fixed inset-y-0 flex flex-col w-[360px] h-screen rounded-l-md overflow-hidden flex-initial ${cartTrayOpen ? 'right-0' : 'right-[-500px]'} transition-[right] duration-500 bg-white`}>
                                <div className="flex justify-between items-center w-full h-fit px-4 py-3 bg-stone-800">
                                    <span className="w-fit h-fit font-inter font-[500] text-[13px] sm:text-[14px] leading-6 tracking-tight text-stone-50">Cart</span>
                                    <button onClick={() => setCartTrayOpen(false)} type="button" aria-label="close cart" className="flex w-fit h-fit rounded-full group focus:outline-none">
                                            ...
                                            cartData.map((eachCartItem: any, index: number) => (
                                                <div key={`cartitem${index}`} className="flex w-full h-fit py-2 border-t border-[#dddbda] gap-1">
                                                    ...
                                                    <div className="flex flex-col w-full h-fit px-2 py-1 gap-1 text-left font-inter font-[500] text-[12px] sm:text-[13px] leading-5 tracking-tight text-stone-600 break-words">
                                                        ...
                                                        <p className="w-full h-fit font-[600] text-stone-900 tracking-tight text-[12px]">{`BDT ${currencyFormatBDT(eachCartItem.item_price)} x ${eachCartItem.item_quantity} = ${currencyFormatBDT(eachCartItem.item_price * eachCartItem.item_quantity)}`}</p>
                                                    </div>
                                                </div>
                                            ))
                                            :
                                ...
                                <div className="flex flex-col w-full h-fit px-4 py-3 bg-white border-t border-stone-300 gap-1">
                                    <div className="flex justify-between w-full h-fit gap-1 py-1">
                                        ...
                                        <div className="w-fit h-fit text-[13px] sm:text-[14px] font-[500] text-stone-800 tracking-tight">
                                            {`${cartData && cartData.length > 0 ? currencyFormatBDT(cartData.reduce((acc, item) => acc + (item.item_price * item.item_quantity), 0)) : 0} BDT`}
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex w-full h-fit gap-1 py-px">
                                        ...
                                        <Link href="/checkout"
                                            className={`flex justify-center w-full h-fit px-6 py-2 text-[13px] sm:text-[14px] leading-5 font-[500] text-white rounded-[20px] focus:outline-none transition-colors duration-100 ease-linear ${(cartData && cartData.length > 0) ? `bg-stone-800 hover:bg-stone-900` : `pointer-events-none bg-stone-600`}`}
                                        >
                                            Checkout
                                        </Link>
                                    </div>
