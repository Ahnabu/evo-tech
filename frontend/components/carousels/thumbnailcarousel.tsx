...
import { m, AnimatePresence, usePresence, LazyMotion, domAnimation, Variants } from "framer-motion";
    ...
    return (
        <LazyMotion features={domAnimation} strict>
            <m.div
                ...
                animate="visible"
                exit="exit"
                className="relative w-full h-full rounded-[8px] overflow-hidden bg-white"
            >
                <Image
...
const ThumbnailCarousel = ({ imgdata, uniqueid }: { imgdata: any[]; uniqueid: string; }) => {

    const [selectedImg, setSelectedImg] = useState<any>(imgdata[0]);
    const [hovering, setHovering] = useState<boolean>(false);
    const [myCursorPosition, setMyCursorPosition] = useState({ x: -100, y: -100 });
    ...
    return (
        <div className="flex flex-col items-center w-full max-w-[550px] h-fit gap-4 sm:gap-5 pb-3 sm:pb-5">
            ...
            >
                ...
                {
                    hovering &&
                    <ZoomLens cursorPosition={myCursorPosition} zoomFactor={1.4} lensSize={360}>
                        <Image
                ...
                >
                    {
                        imgdata.map((image: any, index: number) => (
                            <SwiperSlide key={`thumbnail_img${index}`} className="relative !w-[40px] sm:!w-[60px] h-full">
                                <div className={`relative w-full h-full rounded-[6px] overflow-hidden focus:outline-none cursor-pointer bg-white border-2 ${selectedImg.imgid === image.imgid ? 'border-stone-500 brightness-100' : 'border-stone-100 brightness-[0.8]'}`}
                    ...
                    <button type="button" aria-label="previous button for thumbnail carousel" className="sw-thumbnail-custom-prev z-10 absolute top-[50%] translate-y-[-50%] left-0 flex w-fit h-fit text-stone-400 hover:text-stone-600 rounded-full overflow-hidden">
                        <FiChevronLeft className="w-7 h-7 sm:w-8 sm:h-8 py-1 pr-0.5" />
                    </button>

                    <button type="button" aria-label="next button for thumbnail carousel" className="sw-thumbnail-custom-next z-10 absolute top-[50%] translate-y-[-50%] right-0 flex w-fit h-fit text-stone-400 hover:text-stone-600 rounded-full overflow-hidden">
