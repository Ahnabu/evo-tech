...
const getHeroSections = async (): Promise<HeroSectionDisplayType[]> => {
    ...
    // noStore();
    const heroSectionsData = await axioswithIntercept.get(`/api/admin/lp/topcarousel`)
        .then((res) => res.data.topcarousel_data)
        .catch((error: any) => {
    ...
    return (
        <div className="w-full h-fit flex flex-col px-5 md:px-7 py-8 gap-6 font-inter">
            <div className="flex flex-col gap-4">
                <h2 className="text-lg lg:text-xl font-bold tracking-tight text-stone-900">Hero Section</h2>
                <AddHeroSectionForm />
            </div>
