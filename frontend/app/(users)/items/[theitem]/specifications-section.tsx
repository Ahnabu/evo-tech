...
const ItemSpecsSection = ({ ispecsdata, framerSectionVariants }: { ispecsdata: any[]; framerSectionVariants: Variants; }) => {
    ...
    return (
        ...
        >
            <h1 className="w-fit h-fit py-4 text-center text-[20px] leading-7 sm:text-[22px] sm:leading-8 lg:text-[27px] lg:leading-10 tracking-tight font-[600] text-stone-800 first-letter:text-[24px] sm:first-letter:text-[28px] first-letter:text-[#0866FF]">Specifications</h1>
            ...
            {ispecsdata.length > 0 &&
                <div className="grid grid-cols-2 lg:grid-cols-3 grid-flow-row w-full h-fit p-4 gap-x-7 gap-y-4 min-[700px]:gap-y-7">
                    {ispecsdata.map((spec, index) => (
                        ...
                        >
                            <p className="max-[699px]:col-start-1 w-full h-fit text-[12px] min-[700px]:text-[13px] leading-5 font-[500] text-stone-400">{`${spec.label}`}</p>
