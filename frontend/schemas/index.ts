...
export const checkoutSchema = z.object({
    ...
    shippingType: z.enum(["regular_delivery", "express_delivery", "pickup_point"], {
    ...
    terms: z.boolean().refine(value => value === true, { message: "Accept terms and conditions to proceed" }),
    ...
    if (data.shippingType === 'express_delivery') {
        if (data.paymentMethod !== "bkash" && data.paymentMethod !== "bank_transfer") {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                message: "Select bKash or Bank Transfer as payment method for express delivery",
                path: ["paymentMethod"],
            });
        }
    }
    ...
    if (data.shippingType === 'regular_delivery') {
        if (data.paymentMethod !== "cod" && data.paymentMethod !== "bkash" && data.paymentMethod !== "bank_transfer") {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
    ...
    if (data.shippingType === 'pickup_point') {
        if (data.paymentMethod !== "cop" && data.paymentMethod !== "bkash" && data.paymentMethod !== "bank_transfer") {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
    ...
    if (data.paymentMethod === 'bkash' || data.paymentMethod === 'bank_transfer') {
        if (!data.transactionId || data.transactionId === "" || data.transactionId.trim() === '') {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                message: "Transaction ID is required for the selected payment method",
                path: ["transactionId"],
            });
        }
    }
