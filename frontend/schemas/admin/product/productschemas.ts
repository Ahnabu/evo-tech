...
export const AddProductSchema = z.object({
    ...
    item_brand: z.string()
        .min(1, { message: 'Item brand is required' }),
    ...
    landing_section_id: z.preprocess(
        (arg) => {
            if (arg === null || arg === undefined || arg === "") return null;
            return arg;
        },
    ...
    additional_images: z.preprocess(
        (value) => {
            // Handle FileList
            if (value instanceof FileList) {
...
export const UpdateProductSchema = z.object({
    ...
    item_slug: z.string().min(1, "Item slug is required").max(255, "Item slug must be less than 255 characters"),
    item_price: z.string()
        .min(1, { message: 'Price is required' })
        .refine((val) => {
            const n = Number(val);
    ...
    additional_newimages: z.preprocess(
        (value) => {
            if (value instanceof FileList) {
                const filesArray = Array.from(value);
                return filesArray;
